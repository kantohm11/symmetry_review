...
\end{align}
where $\{\theta\}$ is the number in $[0,2\pi)$ and $\{\theta\} = \theta \mod 2\pi$. Note that the modification is not $\{theta_R-\theta_L\}$, so that the modification is defined *locally* -- it is a sum of independent contributions from the left and right boundaries.
